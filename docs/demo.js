(() => {
  ...
  const finalHTML = [
    ...
    `<span class="phase">— Phase 2: Critique —</span>`,
    `<span class="warn">🎭</span> Codex assigned Devil’s Advocate`,
  ...
  const setStep = (name) => {
    root.querySelectorAll(".demo-rail span").forEach((el) => {
      ...
      if (["cmd", "ping", "plan", "critique", "done"].indexOf(el.dataset.step) <=
          ["cmd", "ping", "plan", "critique", "done"].indexOf(name)) {
  ...
  const typeLine = async (html, charDelay = 18) => {
    // Type plain text approximation then swap to HTML when done for that line
  ...
  const run = async () => {
    for (;;) {
      ...
      setStep("critique");
      await appendLine(`<span class="phase">— Phase 2: Critique —</span>`, 400);
      await appendLine(`<span class="warn">🎭</span> Codex assigned Devil’s Advocate`, 700);
      await appendLine(`<span class="ok">Valid critiques: 4</span>`, 520);
